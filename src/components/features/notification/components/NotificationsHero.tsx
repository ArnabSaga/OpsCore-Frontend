...
const NotificationsHero = ({ total, unread, archived }: Props) => {
  return (
    <section
        ...
        <div className="max-w-3xl space-y-4">
          ...
          <div className="space-y-3">
            <h1 className="text-3xl font-bold tracking-tight text-white md:text-4xl">
              Stay on top of workspace activity
            </h1>
            <p className="max-w-2xl text-sm leading-6 text-[#94A3B8] md:text-base">
              Review task, invitation, invoice, and system events from one central inbox.
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3 xl:min-w-[520px]">
          {[
            ...
            <div key={item.label} className="rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-xs uppercase tracking-[0.16em] text-[#667085]">{item.label}</p>
